...
class SadTalker():
    ...
    def __init__(self, checkpoint_path='checkpoints', config_path='src/config', lazy_load=False):
        ...
        self.sadtalker_paths = init_path(self.checkpoint_path, self.config_path, 256, False, 'full')
        
        # Initialize models only if not lazy loading
        if not lazy_load:
            self.audio_to_coeff = Audio2Coeff(self.sadtalker_paths, self.device)
            self.preprocess_model = CropAndExtract(self.sadtalker_paths, self.device)
            self.animate_from_coeff = AnimateFromCoeff(self.sadtalker_paths, self.device)

    def test_with_preloaded_models(self, source_image, driven_audio, preprocess='full', 
        still_mode=False, use_enhancer=False, batch_size=1, size=256, 
        pose_style=0, exp_scale=1.0, use_ref_video=False,
        ref_video=None, ref_info=None, use_idle_mode=False,
        length_of_audio=0, use_blink=True, result_dir='./results/'):
        """Version of test that uses preloaded models"""
        if not hasattr(self, 'preprocess_model') or not hasattr(self, 'audio_to_coeff') or not hasattr(self, 'animate_from_coeff'):
            raise RuntimeError("Models must be preloaded before using test_with_preloaded_models")
        ...
        ref_pose_coeff_path = ref_video_coeff_path if use_ref_video and ref_info in ['pose', 'pose+blink'] else None
        ref_eyeblink_coeff_path = ref_video_coeff_path if use_ref_video and ref_info in ['blink', 'pose+blink'] else None
    ...
    def test(self, source_image, driven_audio, preprocess='crop', 
        ...
        ref_video=None, ref_info=None, use_idle_mode=False,
        length_of_audio=0, use_blink=True, result_dir='./results/'):
        """Original test method that initializes models if not preloaded"""
        if self.lazy_load and hasattr(self, 'preprocess_model') and hasattr(self, 'audio_to_coeff') and hasattr(self, 'animate_from_coeff'):
            # Use preloaded models if available
            return self.test_with_preloaded_models(
                source_image, driven_audio, preprocess, still_mode, use_enhancer,
                batch_size, size, pose_style, exp_scale, use_ref_video,
                ref_video, ref_info, use_idle_mode, length_of_audio, use_blink, result_dir
            )
            
        # Original initialization code for non-lazy loading
        ...
        # Don't delete models if they were preloaded
